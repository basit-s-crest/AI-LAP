...
async function main() {
  ...
  sRedis.on("message", async (_ch, raw) => {
    try {
      ...
      // ── Flush complete timing hash to Postgres via FastAPI ────────────────
      // All 6 stages are now in Redis — persist them to pipeline_runs table
      fetch(`${FASTAPI_URL}/v1/pipeline/flush/${result.event_id}`, { method: "POST" })
        .then(r => r.json())
        .then(j => {
          if (j.stages_written) {
            // silent success — summary will show DB counts
          }
        })
        .catch(e => console.error(`[DB flush] event_id=${result.event_id}`, e.message));
