...
export default function LiveSessionTranscript({
  ...
  const [insightsTab, setInsightsTab] = useState<"video" | "speech">("video");
  ...
  }, [isRecordingActive, hasRemoteStream, startMemberListening, stopMemberListening]);
        ...
        >
          <div className="space-y-4">
            ...
            <div className="flex justify-center p-0.5 bg-[#E4ECF4]/60 rounded-xl border border-[#D2DBE3]/50 shrink-0">
              <button
                type="button"
                onClick={() => setInsightsTab("video")}
                className={`flex-1 flex items-center justify-center gap-1.5 py-1.5 rounded-lg text-[10px] font-bold font-outfit uppercase tracking-wider transition-all duration-150 ${
                  insightsTab === "video"
                    ? "bg-white text-[#3A6E99] shadow-sm font-bold border border-[#D2DBE3]/30"
                    : "text-[#5C6B73] hover:text-[#1E252B] hover:bg-white/30"
                }`}
              >
                <Activity size={12} />
                <span>Behavior (Video)</span>
              </button>
              <button
                type="button"
                onClick={() => setInsightsTab("speech")}
                className={`flex-1 flex items-center justify-center gap-1.5 py-1.5 rounded-lg text-[10px] font-bold font-outfit uppercase tracking-wider transition-all duration-150 ${
                  insightsTab === "speech"
                    ? "bg-white text-[#4E8C58] shadow-sm font-bold border border-[#D2DBE3]/30"
                    : "text-[#5C6B73] hover:text-[#1E252B] hover:bg-white/30"
                }`}
              >
                <Waves size={12} />
            ...
            {insightsTab === "video" ? (
              /* Video/Behavior Analysis Panel */
              <div className="bg-white border border-[#D2DBE3] rounded-2xl p-4 shadow-sm space-y-4">
                <div className="flex items-center justify-between border-b border-[#D2DBE3]/50 pb-2">
                  <div className="flex items-center gap-1.5 text-xs font-outfit font-bold text-[#3A6E99]">
                    ...
                    <span>EMOTION & PRESENCE SIGNALS</span>
                  </div>
                ...
                {isDev && (
                  <div className="border-t border-[#D2DBE3]/50 pt-3 mt-3 space-y-2">
                    <div className="text-[10px] font-sans font-bold text-[#3A6E99] uppercase tracking-wider flex items-center gap-1">
                      <span>🛠️</span>
                  ...
                  <>
                    ...
                    <div className="space-y-4 relative before:absolute before:left-[17px] before:top-2 before:bottom-2 before:w-[2px] before:bg-gradient-to-b before:from-[#D2DBE3] before:to-[#D2DBE3]/30">
                      {[...insightsLog].reverse().map((entry, idx) => {
                        ...
                        const affectStyle = entry.tone ? (AFFECT_STYLES[entry.tone.affect_label.toLowerCase()] || AFFECT_STYLES.neutral) : null;
                              ...
                              {entry.tone && (
                                <div className="px-4 pt-3">
                                  <div className="grid grid-cols-4 gap-2">
                                    ...
                                    {/* Tone Category */}
                                    {(() => {
                                      const label = entry.tone.affect_label?.toLowerCase() || 'neutral';
                                      const style = AFFECT_STYLES[label] || AFFECT_STYLES.neutral;
                                      return (
                                        <div className={`rounded-lg p-2 text-center border capitalize ${style.bg} ${style.border}`}>
                                          <div className="text-[8px] font-sans font-semibold text-soft uppercase tracking-wider">Tone</div>
                                          <div className={`text-[12px] font-outfit font-bold ${style.text} flex items-center justify-center gap-1`}>
