...
exports.getAiSessionNoteBySessionId = exports.createAiSessionNote = void 0;
...
function formatEmotionDataForPrompt(emotionCounts, emotionTimeline) {
    if (!emotionCounts && !emotionTimeline)
        return "";
...
function generateMockAnalysis(transcript, memberName, emotionCounts, emotionTimeline) {
    ...
    if (emotionCounts && Object.keys(emotionCounts).length > 0) {
        const dominant = Object.entries(emotionCounts).reduce((a, b) => b[1] > a[1] ? b : a)[0];
...
async function runAnthropicAnalysis(formattedTranscript, apiKey, emotionCounts, emotionTimeline) {
...
const createAiSessionNote = async (req, res) => {
    try {
        const coachId = req.user?.id;
        if (!coachId) {
            return res.status(401).json({ message: "Unauthorized" });
        }
        const { memberId, sessionId, transcript, emotionTimeline, emotionCounts } = req.body;
...
Respond with ONLY the JSON object. Do not wrap in markdown \`\`\`json blocks.`
        ...
        const savedNote = await prisma_1.default.aiSessionNote.create({
            data: {
...
exports.createAiSessionNote = createAiSessionNote;
const getAiSessionNoteBySessionId = async (req, res) => {
    try {
        const coachId = req.user?.id;
        if (!coachId) {
            return res.status(401).json({ message: "Unauthorized" });
        ...
        const note = await prisma_1.default.aiSessionNote.findFirst({
...
exports.getAiSessionNoteBySessionId = getAiSessionNoteBySessionId;
