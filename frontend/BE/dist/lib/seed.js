...
const runSeed = async () => {
    ...
    const hashedPassword = await (0, auth_service_1.hashPassword)("demo1234");
    const testUser = await prisma_1.default.user.upsert({
    ...
    const coachUser = await prisma_1.default.user.upsert({
    ...
    const groupTwo = await prisma_1.default.communityGroup.findFirst({
    ...
    const existingMessages = await prisma_1.default.message.count({
        where: {
            OR: [
    ...
    if (existingMessages === 0) {
        await prisma_1.default.message.createMany({
            data: [
                {
