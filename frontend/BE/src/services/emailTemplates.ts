...
export interface AppEmailBranding {
  brandTitle: string;
  brandTagline: string;
  primaryColor: string;
}
...
export function buildAppEmailHtml(
  content: AppEmailContent,
  branding?: Partial<AppEmailBranding>
): string {
  ...
  const bodyLines = content.lines
    .map(
      (line) =>
        `<p style="margin:0 0 12px;color:${BRAND_BASE.mid};font-size:15px;line-height:1.55;">${escapeHtml(line)}</p>`
    )
    .join("");
