...
async function checkActivityData() {
  ...
  try {
    ...
    const usersWithLastActive = await prisma.user.count({ 
    ...
    const coachesWithLastActive = await prisma.coach.count({ 
    ...
    const orgsWithLastActive = await prisma.organization.count({ 
      where: { lastActiveAt: { not: null } } 
    });
    ...
    const coachesInRange = await prisma.coach.count({
      where: {
        OR: [
          { lastActiveAt: { gte: thirtyDaysAgo } },
          { lastActiveAt: null, createdAt: { gte: thirtyDaysAgo } }
    ...
    const orgsInRange = await prisma.organization.count({
      where: {
        OR: [
          { lastActiveAt: { gte: thirtyDaysAgo } },
          { lastActiveAt: null, createdAt: { gte: thirtyDaysAgo } }
