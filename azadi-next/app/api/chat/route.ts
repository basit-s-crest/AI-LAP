 ...
 * The worker picks it up, calls FastAPI, and publishes the result
 * to Redis pub/sub → SSE stream → dashboard.
 */
import { NextRequest, NextResponse } from "next/server";
import { getChatQueue } from "@/lib/queue";
...
export async function POST(req: NextRequest) {
  try {
    ...
    const jobPayload: ChatJobPayload = {
      ...
      role: "member", // we're analysing the member's message text
      text: text.trim(),
      timestamp,
      consent_active: true,
    ...
    console.log(
      `[API /chat] Enqueued job ${job.id} for member=${member_token}`
    );
